...
function testEnv() {
  ...
  isEqual({
    subTitle: `La variable "SERVER_HOST" debe estar declarada`,
    result: process.env.SERVER_HOST !== undefined,
  });
  isEqual({
    subTitle: `El valor de "SERVER_HOST" debe ser "127.0.0.1"`,
    result: process.env.SERVER_HOST === "127.0.0.1",
  });
  isEqual({
    subTitle: `La variable "SERVER_PORT" debe estar declarada`,
    result: process.env.SERVER_PORT !== undefined,
  });
  isEqual({
    subTitle: `El valor de "SERVER_PORT" debe ser "3005"`,
    result: process.env.SERVER_PORT === "3005",
  });
  isEqual({
    subTitle: `La variable "DATABASE_URL" debe estar declarada`,
    result: process.env.DATABASE_URL !== undefined,
  });
  isEqual({
    subTitle: `El valor de "DATABASE_URL" no debe estar vacío`,
    result: process.env.DATABASE_URL && process.env.DATABASE_URL !== "",
  });
  isEqual({
    subTitle: `La variable "DATABASE_NAME" debe estar declarada`,
    result: process.env.DATABASE_NAME !== undefined,
  });
  isEqual({
    subTitle: `El valor de "DATABASE_NAME" debe ser "muebleria"`,
    result: process.env.DATABASE_NAME === "muebleria",
  });
}
...
async function testEnvDist() {
  ...
  isEqual({
    subTitle: `La variable "SERVER_HOST" debe estar declarada`,
    result: SERVER_HOST !== undefined,
  });
  isEqual({
    subTitle: `El valor de "SERVER_HOST" debe ser "127.0.0.1"`,
    result: SERVER_HOST && SERVER_HOST[1].trim() === "127.0.0.1",
  });
  isEqual({
    subTitle: `La variable "SERVER_PORT" debe estar declarada`,
    result: SERVER_PORT !== undefined,
  });
  isEqual({
    subTitle: `El valor de "SERVER_PORT" debe ser "3005"`,
    result: SERVER_PORT && SERVER_PORT[1].trim() === "3005",
  });
  isEqual({
    subTitle: `La variable "DATABASE_URL" debe estar declarada`,
    result: DATABASE_URL !== undefined,
  });
  isEqual({
    subTitle: `El valor de "DATABASE_URL" debe ser "tu-cadena-de-conexion"`,
    result: DATABASE_URL && DATABASE_URL[1].trim() === "tu-cadena-de-conexion",
  });
  isEqual({
    subTitle: `La variable "DATABASE_NAME" debe estar declarada`,
    result: DATABASE_NAME !== undefined,
  });
  isEqual({
    subTitle: `El valor de "DATABASE_NAME" debe ser "muebleria"`,
    result: DATABASE_NAME && DATABASE_NAME[1].trim() === "muebleria",
  });
}
...
async function testGitIgnore() {
  ...
  isEqual({
    subTitle: `El directorio "node_modules/" debe estar declarado`,
    result: NODE_MODULES && NODE_MODULES.trim() === "node_modules/",
  });
  isEqual({
    subTitle: `El archivo ".env" debe estar declarado`,
    result: ENV && ENV.trim() === ".env",
  });
}
...
async function testFileConnectionDB() {
  ...
  isEqual({
    subTitle: `El archivo "connection_db.js" debe existir`,
...
async function run() {
  ...
  await testMethodPOST(
    ...
    (response) => ({
      subTitle: `El código HTTP debe ser 201`,
  ...
  await testMethodPOST(
    ...
    (response) => ({
      subTitle: `El código HTTP debe ser 400`,
  ...
  await testMethodPUT(
    ...
    (response) => ({
      subTitle: `El código HTTP debe ser 200`,
  ...
  await testMethodGET(
    ...
    [
      (muebles) => ({
        subTitle: `La cantidad de muebles debe ser 3`,
        result: muebles.length === 3,
      }),
      (muebles) => ({
        subTitle: `El orden de códigos debe ser 9, 4 y 6`,
        result:
          muebles.map((mueble) => mueble.codigo).join() === [9, 4, 6].join(),
      }),
    ]
  );
  ...
  await testMethodGET(
    ...
    [
      ...
      (muebles) => ({
        ...
        result:
          muebles.map((mueble) => mueble.codigo).join() === [3, 8, 13].join(),
      }),
    ]
  );
  ...
  await testMethodGET(
    ...
    [
      (muebles) => ({
        subTitle: `La cantidad de muebles debe ser 4`,
        result: muebles.length === 4,
      }),
      (muebles) => ({
        subTitle: `El orden de códigos debe ser 5, 1, 14 y 6`,
        result:
          muebles.map((mueble) => mueble.codigo).join() ===
          [5, 1, 14, 6].join(),
      }),
    ]
  );
